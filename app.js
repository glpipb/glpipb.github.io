...
async function showFormModal(type, category = null) {
    ...
    switch (type) {
        ...
        case 'credentials':
            ...
            for (const [key, field] of Object.entries(config.fields)) {
                if (key === 'inventoryNumber') continue;
    ...
    document.getElementById(formId).addEventListener('submit', async (e) => {
        ...
        if (type === 'inventory') {
            const counterRef = db.collection('counters').doc('inventoryCounter');
            try {
                const newInventoryNumber = await db.runTransaction(async (transaction) => {
                    const counterDoc = await transaction.get(counterRef);
                    if (!counterDoc.exists) { throw "El documento contador de inventario no existe."; }
                    const newNumber = counterDoc.data().currentNumber + 1;
                    transaction.update(counterRef, { currentNumber: newNumber });
                    return newNumber;
                });
                data.inventoryNumber = newInventoryNumber;
            } catch (error) {
                console.error("Error al obtener el número de inventario:", error);
                alert("No se pudo asignar un código de inventario. Revisa la consola.");
                return;
            }
        }
