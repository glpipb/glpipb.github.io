...
// En la sección de TEMPLATES HTML
const statisticsHTML = `
    ...
    <!-- Contenedor para los errores (inicialmente oculto) -->
    <div id="stats-error-container" class="card" style="display:none; color: red;"></div>

    <!-- Contenedor para el contenido de los reportes -->
    <div id="stats-content-wrapper">
        <h2>Análisis de Tickets</h2>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 25px;">
            <div class="card"><h3>Tickets por Prioridad</h3><canvas id="ticketsByPriorityChart"></canvas></div>
            <div class="card"><h3>Tickets por Categoría de Dispositivo</h3><canvas id="ticketsByDeviceCategoryChart"></canvas></div>
        ...
        <div class="card"><h3>Flujo de Tickets (Creados vs. Cerrados)</h3><canvas id="ticket-flow-chart"></canvas></div>
        <h2 style="margin-top: 40px;">Resumen de Inventario</h2>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 25px;">
            <div class="card"><h3>Dispositivos por Categoría</h3><canvas id="inventoryByCategoryChart"></canvas></div>
            <div class="card"><h3>Computadores por SO</h3><canvas id="computersByOsChart"></canvas></div>
        </div>
    </div>`;
...
async function renderEstadisticas(container) {
    ...
    generateReports(); // La llamada inicial
}
...
async function showItemFormModal(type, category = null, docId = null) {
    ...
    switch (type) {
        case 'inventory':
        case 'credentials':
            const configObject = type === 'inventory' ? inventoryCategoryConfig : credentialsCategoryConfig;
