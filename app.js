...
async function renderEstadisticas(container) {
    // El único cambio está en la variable 'statisticsHTML' de abajo
    const statisticsHTML = `
    ...
    container.innerHTML = statisticsHTML; // Inyecta el HTML corregido
    ...
    const generateReports = async () => {
        ...
        try {
            ...
            charts.ticketsByPriority = new Chart(chartContexts.ticketsByPriority, {
                type: 'doughnut',
                data: { labels: Object.keys(priorityCounts), datasets: [{ data: Object.values(priorityCounts), backgroundColor: ['#007bff', '#ffc107', '#dc3545'] }] },
            ...
            topDevicesList.innerHTML = topDevices.map(([id, count]) => {
                const device = inventoryMap[id];
                return `<li><span>${device ? `${device.brand} ${device.model}` : id}</span><span class="kpi-count">${count}</span></li>`;
            ...
            // Lógica adicional para los otros gráficos...
    ...
    generateReports(); // Llama para cargar los datos iniciales
}
