...
const inventoryCategoryConfig = {
    ...
    printers: { title: 'Impresoras', titleSingular: 'Impresora', prefix: 'IMP-', counter: 'impresoraCounter', fields: { id: { label: 'Código' }, brand: { label: 'Marca', type: 'text' }, model: { label: 'Modelo', type: 'text' }, serial: { label: 'N/Serie', type: 'text' }, ipAddress: { label: 'Dirección IP', type: 'text' }, type: { label: 'Tipo (Láser, Tinta)', type: 'text' } }}
};
...
function renderMaintenanceCalendar(container) {
    ...
    db.collection('maintenance').where('status', 'in', ['planificada', 'completada']).onSnapshot(snapshot => {
        const eventColors = {
            'Preventivo': '#dc3545',
            'Correctivo': '#ffc107',
            'Mantenimiento Lógico': '#6f42c1', // Color para Mantenimiento Lógico
            'Backup': '#fd7e14', // Color para Backup
            'Tarea': '#007bff',
            'Recordatorio': '#17a2b8'
        };
...
async function showFormModal(type, category = null) {
    ...
    switch (type) {
        case 'inventory':
        case 'credentials':
            configObject = type === 'inventory' ? inventoryCategoryConfig : credentialsCategoryConfig;
            config = configObject[category];
            title = `Añadir ${config.titleSingular}`;
            collectionName = type;
            let fieldsHTML = '';
            for (const [key, field] of Object.entries(config.fields)) {
                ...
                if (field.type === 'textarea') inputHTML = `<textarea id="form-${key}" name="${key}" rows="3"></textarea>`;
                    ...
                    if (field.optionsSource === 'locations') {
                        ...
                        optionsHTML += locSnap.docs.map(doc => `<option value="${doc.id}">${doc.id}: ${doc.data().name}</option>`).join('');
                    } else {
        ...
        case 'maintenance':
            ...
            formHTML = `<div class="form-group"><label for="form-task">Título de la Tarea</label><input type="text" id="form-task" name="task" required></div><div class="form-group"><label for="form-date">Fecha</label><input type="date" id="form-date" name="date" required></div><div class="form-group"><label for="form-type">Tipo de Tarea</label><select id="form-type" name="type"><option value="Mantenimiento Preventivo">Mantenimiento Preventivo</option><option value="Mantenimiento Correctivo">Mantenimiento Correctivo</option><option value="Mantenimiento Lógico">Mantenimiento Lógico</option><option value="Backup">Backup</option><option value="Tarea">Tarea</option><option value="Recordatorio">Recordatorio</option></select></div>`;
            break;
    }
    ...
    document.getElementById(formId).addEventListener('submit', async (e) => {
        ...
        if (type === 'inventory' || type === 'credentials') {
            ...
            if (!prefix || !counter) {
                alert('Error de configuración: falta prefijo o contador para esta categoría.');
                return;
            }
            ...
            try {
                const newId = await db.runTransaction(async (transaction) => {
                    const counterDoc = await transaction.get(counterRef);
                    if (!counterDoc.exists) { throw `El documento contador '${counter}' no existe.`; }
                    const newNumber = counterDoc.data().currentNumber + 1;
                    transaction.update(counterRef, { currentNumber: newNumber });
                    return `${prefix}${newNumber}`;
                });
                ...
                console.error("Error al generar ID y guardar:", error);
                alert("No se pudo asignar un código. Revisa la consola.");
                return;
            }
        } else {
    ...
    let formHTML = `
        ...
        <div class="form-group">
            ...
            <select id="form-type" name="type">
                <option value="Mantenimiento Preventivo" ${taskType === 'Mantenimiento Preventivo' ? 'selected' : ''}>Mantenimiento Preventivo</option>
                <option value="Mantenimiento Correctivo" ${taskType === 'Mantenimiento Correctivo' ? 'selected' : ''}>Mantenimiento Correctivo</option>
                <option value="Mantenimiento Lógico" ${taskType === 'Mantenimiento Lógico' ? 'selected' : ''}>Mantenimiento Lógico</option>
                <option value="Backup" ${taskType === 'Backup' ? 'selected' : ''}>Backup</option>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    appContent.addEventListener('click', e => {
        ...
        if (target.matches('.edit-btn, .edit-btn *')) {
            ...
            row.querySelectorAll('td[data-field]:not([data-field="id"]), span.config-item-name').forEach(cell => {
                ...
                if (fieldConfig.type === 'select') {
                    ...
                    if (fieldConfig.optionsSource === 'locations') {
                        db.collection('locations').get().then(snap => {
                            snap.forEach(doc => {
                                optionsHTML += `<option value="${doc.id}" ${doc.id === currentText ? 'selected' : ''}>${doc.id}: ${doc.data().name}</option>`;
                            });
